...
class updateViewController: UIViewController , UITableViewDataSource , UITableViewDelegate{
    ...
    func loadData()
    {
        dataConnection.retrieve_post(postcategory: "Updates", completion:
            { message in
                if  message == "Done"
                {
    ...
    func deletePost(id: String)
    {
        ...
        let restartaction = UIAlertAction(title: "Delete", style: .destructive, handler: { (UIAlertAction) in
            ...
            self.dataConnection.deletePost(category: "Updates" , child: "\(id)" , completion:
                { message in
...
extension updateViewController : IndicatorInfoProvider
{
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "Updates")
    }
}
